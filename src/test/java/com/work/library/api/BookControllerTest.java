...
class BookControllerTest {
    ...
    class Update {
        ...
        @Test
        void 도서_대여에_성공하면_200_응답을_반환한다() throws Exception {
            Long bookId = 1L;

            when(bookUpdateApplication.rental(bookId))
                    .thenReturn(bookId);

            mockMvc.perform(
                    post("/v1/books/{bookId}/rental", 1L)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.type").value(ResponseType.SUCCESS.name()))
                    .andExpect(jsonPath("$.message").value(ResponseMessage.BookResponseMessage.SUCCESS_BOOK_CHANGE))
                    .andExpect(jsonPath("$.data.id").value(1L))
                    .andDo(document("book-rental-success",
                            preprocessRequest(prettyPrint()),
                            preprocessResponse(prettyPrint()),
                            pathParameters(
                                    parameterWithName("bookId").description("대여할 도서 ID")
                            ),
                            responseFields(
                                    fieldWithPath("type").type(STRING).description("응답 타입"),
                                    fieldWithPath("message").type(STRING).description("응답 메시지"),
